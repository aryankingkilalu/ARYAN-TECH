...
global.botname = settings.botName || "Teddy-X";
global.themeemoji = "•";
...
// Teddy_Sessions System Start
if (!fs.existsSync('./aryan_Sessions')) {
    fs.mkdirSync('./Aryan_Sessions', { recursive: true });
    console.log(chalk.green('📁 Created aryan_Sessions directory'));
}
...
async function downloadSessionFromMega() {
    return new Promise((resolve, reject) => {
        ...
        console.log(chalk.blue('📥 Downloading Aryan_Sessions from mega.nz...'));
        try {
            const sessdata = global.SESSION_ID.replace("aryan-xmd:~", '');
            ...
            filer.download((err, data) => {
                ...
                fs.writeFile('./aryan_Sessions/creds.json', data, (writeErr) => {
                    if (writeErr) {
                    ...
                    console.log(chalk.green('✅ aryan_Sessions downloaded successfully'));
                    resolve(true);
                });
            });
        } catch (error) {
...
async function checkAndDownloadSession() {
    if (!fs.existsSync('./aryan_Sessions/creds.json')) {
...
async function startHansTechInc() {
    try {
        ...
        const { state, saveCreds } = await useMultiFileAuthState('./Teddy_Sessions');
        ...
        const HansTechInc = makeWASocket({
            ...
            printQRInTerminal: !fs.existsSync('./aryan_Sessions/creds.json'),
        ...
        HansTechInc.ev.on('connection.update', async (s) => {
            ...
            if (connection == "open") {
                ...
                await delay(1999);
                console.log(chalk.yellow(`\n\n                  ${chalk.bold.blue(`[ ${global.botname || 'aryan-X'} ]`)}\n\n`));
                console.log(chalk.cyan(`< ================================================== >`));
                console.log(chalk.magenta(`\n${global.themeemoji || '•'} YT CHANNEL: ARYAN`));
                console.log(chalk.magenta(`${global.themeemoji || '•'} GITHUB: Aryan`));
            ...
            if (connection === 'close') {
                ...
                if (statusCode === DisconnectReason.loggedOut || statusCode === 401) {
                    try {
                        rmSync('./aryan_Sessions', { recursive: true, force: true });
                        console.log(chalk.yellow('🗑️  Aryan_Sessions folder deleted.'));
                ...
                for (const call of calls) {
                    ...
                    try {
                        ...
                        if (!antiCallNotified.has(callerJid)) {
                            ...
                            await HansTechInc.sendMessage(callerJid, { text: '> _📵 Anticall is enabled. Your call was rejected by aryan xmd bot and you will be blocked_' });
                        }
                    } catch {}
                    setTimeout(async () => {
